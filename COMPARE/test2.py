...
def TSNE1(model,dataloaders,filename):
    ...
    with torch.no_grad():
        ...
        for data, target in dataloaders["source_val"]:
            ...
            source_feature.append(feature)
            source_label.append(target)
        for data, target in dataloaders["target_train"]:
            feature = model(data)
...
def main():
    ...
    args_model_name="cnn_features_1d"#"CNN_1d"
    ...
    datasets1['source_train'], datasets1['source_val'], datasets1['target_train'], datasets1['target_val'] = Dataset(args_data_dir, args_transfer_task, args_normlizetype).data_split(transfer_learning=True)
    ...
    if args_bottleneck:
        bottleneck_layer = nn.Sequential(nn.Linear(model.output_num(), args_bottleneck_num),
                                                nn.ReLU(inplace=True), nn.Dropout())
        classifier_layer = nn.Linear(args_bottleneck_num, Dataset.num_classes)
    else:
        classifier_layer = nn.Linear(model.output_num(), Dataset.num_classes)

    model_all = nn.Sequential(model, bottleneck_layer, classifier_layer)

    # MK-MMD
    # model_all.load_state_dict(torch.load(r'./checkpoint/cnn_features_1d_0730-113551/7-0.9659-best_model.pth'))
    # acc = test_Model(model_all, dataloaders['target_val'], './confusion_matrix_MK-MMD')
    # TSNE1(model, dataloaders, './TSNE_MK-MMD')

    # JMMD
    # model_all.load_state_dict(torch.load(r'./checkpoint/cnn_features_1d_0730-114232/7-0.9719-best_model.pth'))
    # acc = test_Model(model_all, dataloaders['target_val'], './confusion_matrix_JMMD')
    # TSNE1(model, dataloaders, './TSNE_JMMD')

    # CORAL
    # model_all.load_state_dict(torch.load(r'./checkpoint/cnn_features_1d_0730-114610/5-0.7194-best_model.pth'))
    # acc = test_Model(model_all, dataloaders['target_val'],'./confusion_matrix_CORAL')
    # TSNE1(model,dataloaders,'./TSNE_CORAL')

    # DANN
    # model_all.load_state_dict(torch.load(r'./checkpoint/cnn_features_1d_0730-115008/7-0.9339-best_model.pth'))
    # acc = test_Model(model_all, dataloaders['target_val'],'./confusion_matrix_DANN')
    # TSNE1(model,dataloaders,'./TSNE_DANN')

    # CDAN
    model_all.load_state_dict(torch.load(r'./checkpoint/cnn_features_1d_0730-115247/6-0.8236-best_model.pth'))
    acc = test_Model(model_all, dataloaders['target_val'],'./confusion_matrix_CDAN')
    TSNE1(model,dataloaders,'./TSNE_CDAN')
