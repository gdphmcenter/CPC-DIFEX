...
class CWRU(object):
    ...
    def data_split(self, transfer_learning=True):
        if transfer_learning:
            ...
            #get source train and val
            list_data = get_files1(self.data_dir, self.source_N)
            data_pd = pd.DataFrame({"data": list_data[0], "label": list_data[1]})
            train_pd, val_pd = train_test_split(data_pd, test_size=0.2, random_state=40, stratify=data_pd["label"])
            source_train = dataset(list_data=train_pd, transform=self.data_transforms['train'])
            source_val = dataset(list_data=val_pd, transform=self.data_transforms['val'])
            ...
            # get target train and val
            list_data = get_files1(self.data_dir, self.target_N)
            data_pd = pd.DataFrame({"data": list_data[0], "label": list_data[1]})
...
# #Digital data was collected at 12,000 samples per second
# signal_size = 1024
# dataname= {0:["97.mat","105.mat","118.mat","130.mat"],  
#            1:["100.mat","108.mat","121.mat","133.mat"]} 

# datasetname = ["source domain", "target domain"]
...
#             train_pd_target_train, val_pd_target_test = train_test_split(data_pd_target_train, test_size=0.3, random_state=40, stratify=data_pd_target_train["label"])
#             val_pd_target_test = pd.concat([val_pd_target_test, data_pd_target_test])

#             target_train = dataset(list_data=train_pd_target_train, transform=self.data_transforms['train'])
#             target_val = dataset(list_data=val_pd_target_test, transform=self.data_transforms['val'])
