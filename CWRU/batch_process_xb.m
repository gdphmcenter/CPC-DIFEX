...
for i=1:m
    ...
    fname=['小波时频10/train_img/',num2str(i),'-',num2str(label(i)),'.jpg'];
    saveas(gcf,fname);
end
%% 验证集
[m,n]=size(valid_X);
...
for i=1:m
    ...
    imagesc(t,f,abs(coefs)/max(max(abs(coefs))));
    set(gca,'position',[0 0 1 1])
    fname=['小波时频10/valid_img/',num2str(i),'-',num2str(label(i)),'.jpg'];
    saveas(gcf,fname);
end
%% 测试集
[m,n]=size(test_X);
...
for i=1:m
    ...
    imagesc(t,f,abs(coefs)/max(max(abs(coefs))));
    set(gca,'position',[0 0 1 1])
    fname=['小波时频10/test_img/',num2str(i),'-',num2str(label(i)),'.jpg'];
    saveas(gcf,fname);
end